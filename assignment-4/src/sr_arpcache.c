...
void sr_arpcache_handle_arpreq(struct sr_instance *sr, struct sr_arpreq *req)
{
    ...
    */
    ...
    if (difftime(curtime, req->sent) > 0.5)         /* sleep(1.0), this seem to be protective coding */
    {
        ...
        if (req->times_sent >= 5)
        {
            ...
            for (pck = req->packets; pck != NULL; pck = pck->next)
            {
                ...
                entry = sr_arpcache_lookup(cache, (rtentry->gw.s_addr != 0) ? rtentry->gw.s_addr : i_hdr0->ip_src);
                ...
                {                    
                    /* queue */
                    struct sr_arpreq *icmp_req = sr_arpcache_queuereq(cache, (rtentry->gw.s_addr != 0) ? rtentry->gw.s_addr : i_hdr0->ip_src, buf, len, rtentry->interface);
                    sr_arpcache_handle_arpreq(sr, icmp_req);
                }
        ...
        else
        {
            ...
            if (rtentry) ifc = sr_get_interface(sr, rtentry->interface);
            else return;
